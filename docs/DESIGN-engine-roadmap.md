...
(§9), the first being: *what does this make the spine do that it has never done?*
...
| **ensemble** | **server tick** (musical) | **yes** | shared, broadcast | N writers, high | **none** (client audio) | text | **shared *time*** — tight cross-device sync; zero host audio |
...
## 3. Why the current slate (snake, canvas, ensemble)
...
1. **No engine had a server clock.** chess/`filter`/`chat` are all *purely reactive* —
   the world is frozen until a client acts. Nothing forced the §5 loop to advance
   state on a timer, buffer asynchronous inputs, or broadcast at a fixed rate — i.e.
   **netcode**. This is the single most consequential unproven claim in the design,
   ...
   (DESIGN §13).
2. **No engine had write contention.** chess avoids it with turns; `filter`/`chat`
   avoid it by being private. Nothing tested *many clients mutating one shared thing
   at once*. → **[`canvas`](DESIGN-canvas.md)** fills it, and surfaces the payoff that
   the §5 single-thread loop hands a shared editor a **free total order** (no
   CRDT/OT). iPad-native.
3. **The synthesis — shared *time*.** Combine an authoritative clock with a shared
   ...
   collaborative music. → **[`ensemble`](DESIGN-ensemble.md)** fills it with the
   shape "stream the score, synthesize locally," adding one reusable primitive
   (clock sync) and **zero** host-side audio.

These three were chosen to **maximize new axis coverage with minimal overlap**: two
add the server clock (one as a game, one as a metronome), two add write contention,
and `ensemble` alone adds tight shared time. Together with the first three, the matrix
(§2) is strikingly complete: turn-based, real-time-private-media, blocking-external,
real-time-authoritative-tick, concurrent-shared-mutation, and synchronized-time are
all covered.
...
- [`DESIGN-villen.md`](DESIGN-villen.md) — the architecture: single binary, single
  loop (§5), WS player edge (§6), in-process admin, the structures that protect
  deferred features (§9). **Read first.**
...
- [`DESIGN-self-hotfix.md`](DESIGN-self-hotfix.md) — forward-looking: could the
  appliance patch its own minor bugs? (A control plane on top of `chat` tool-calling,
  not an engine.)
...
- [`DESIGN-snake.md`](DESIGN-snake.md) — *authoritative clock + netcode.* Design (a
  port of [github.com/aleozlx/snake](https://github.com/aleozlx/snake)).
- [`DESIGN-canvas.md`](DESIGN-canvas.md) — *shared-write contention.* Design.
- [`DESIGN-ensemble.md`](DESIGN-ensemble.md) — *synchronized shared time.* Design.

The generalized `IEngine` slot these all plug into is specified in
[`DESIGN-filter.md`](DESIGN-filter.md) §2 (selection via `--engine chess|filter|chat|
snake|canvas|ensemble`).
...
1. **A pure ruleset in `engine/`, CI-tested against an oracle** — `chess::Position`,
   `filter::process` (vs. CPU reference), `snake::step`, `canvas::apply`,
   `ensemble` transport math. Determinism + tests is how device/blocking code stays
   out of the §9.1 pure core.
2. **§5 single-thread is an asset, not a constraint** — it gives `canvas`/`ensemble` a
   free total order on concurrent writes, and `snake` a lock-free authoritative tick.
   The one engine that genuinely blocks (`chat`) uses the single sanctioned escape
   hatch (a subprocess, fd in `poll()`), nothing more.
3. **Shared-broadcast *or* private-per-connection, both first-class** — chess/snake/
   canvas/ensemble broadcast; filter/chat are private. The slot must assume neither
   (DESIGN-filter §2).
4. **Reuse the edges** — the binary transport (`filter`), the stb codec (`filter` →
   ...
   `snake` AI), the clock-sync primitive (`ensemble` → future timed engines).
5. **The admin is in-process and gamepad-navigable** — every engine draws its own
   `drawAdmin()` panel; the operator runs the cabinet on the Deck.

---

## 6. Runners-up & deferred (recorded so the slate reads as a decision)

- **A lobby / multi-session multiplexer.** Not a topic engine but an *architectural*
  stress: run **several engine instances at once** (many chess games, or chess +
  canvas), attacking the single-hardcoded-`"default"`-session assumption (DESIGN §13
  "multi-session management"). Arguably should come **before** more engines, since all
  of them benefit. Highest-value non-engine next step.
...
- **More media** — audio *voice* (vs. `ensemble`'s synth), depth/second-camera,
  screen-share via `getDisplayMedia`. Each rides the binary transport; none adds a
  *new* axis beyond `filter`, so lower priority.
- **A durability/persistence engine** — a leaderboard, a saved-game library, a wiki.
  More a cross-cutting concern (the §9.2 serializable-state claim) than a flashy
  engine; fold persistence into existing engines first.
